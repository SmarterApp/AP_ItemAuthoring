...
sub run {
  ...
  Hello, $in{firstName}!
    ...
    Your SBAC CDE account has been created. A follow-up email will provide a link to set your password. Please follow the directions in the email and login at: https://${webHost}${orcaUrl}
...
END_HERE
    ...
    my $reviewTypeHtml = &hashToSelect( 'reviewType', \%review_type, '', '',
					'', '', 'width:175px;' );
    my $adminTypeHtml =
...
END_HERE
    ...
    $psgi_out .= <<END_HERE;
		  ...
		  <tr>
			  <td><span class="text">Review Type:</span></td><td>${reviewTypeHtml}</td>
			</tr>
		  <tr>
...
sub print_edit_user {
    ...
    my $reviewTypeHtml =
        &hashToSelect( "reviewType", \%review_type,
            $user->{reviewType}, '',  '', '', 'width:160px;' );
