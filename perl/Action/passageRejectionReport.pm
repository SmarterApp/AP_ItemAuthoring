package Action::passageRejectionReport;
...
sub run {
  ...
  our $requestDir = "${orcaPath}workflow/passage-rejection-report/state-$in{rejectState}";
  make_path( $requestDir ) unless -e $requestDir;

  our $requestFile = "$requestDir/$in{passageId}.html";
  ...
  if ( $in{myAction} eq 'put' ) {
    ...
    <html>
      <head>
        <title>Rejection Report for Passage  $in{passageName}</title>
      </head>
      <body>
        <h3>Rejection Report for Passage $in{passageName}</h3>
...
END_HERE
    ...
    <html>
      <head>
        <title>Client Review Rejection Report for Passage $in{passageName}</title>
      </head>
      <body>
        <h3>Client Review Rejection Report for Passage $in{passageName}</h3>
        <table border="0" cellpadding="2" cellspacing="2">
  		 <tr>
      ...
      <script language="JavaScript">
        function submitParentForm() {
          window.opener.document.form1.submit();
  	      window.opener.parent.menuFrame.location='${orcaUrl}cgi-bin/passageApprove.pl?language=$in{language}&itemBankId=$in{itemBankId}';
          window.close();
        }
...
<html>
  ...
  <body>
   <form name="form1" action="passageRejectionReport.pl" method="POST">
     ...
     <input type="hidden" name="passageName" value="${passageName}" />
