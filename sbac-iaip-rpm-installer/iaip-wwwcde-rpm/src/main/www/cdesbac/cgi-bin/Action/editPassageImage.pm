package Action::editPassageImage; 
...
sub run {
  ...
  my $passage = '';
  if ( $q->param('passage') ne '' ) {
      $passage = $q->param('passage');
  }
  
  my $imageName = $image;
  
  $image = "${passageUrl}lib${itemBankId}/images/p${passage}/${image}";
...
function pxn8_save_image(){
   document.location = "${orcaUrl}cgi-bin/savePassageImage.pl?item_bank_id=${itemBankId}&passage=${passage}&image_name=${imageName}&cached_image=" + PXN8.getUncompressedImage(); }
