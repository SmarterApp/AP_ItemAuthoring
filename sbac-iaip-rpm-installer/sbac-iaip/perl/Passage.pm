...
sub new {
    ...
    if ( $self->{id} > 0 ) {
        ...
        if ( my $row = $sth->fetchrow_hashref ) {
            ...
            where ae.p_id = '. $self->{id} . ' and af.af_type = 1 and af.af_feature = 2';
		    ...
		    while(my $row = $sth->fetchrow_hashref){
		        $self->{accessibilityData} .=  '{"name":"'.$row->{ae_content_name}.'","linkType":'.$row->{ae_content_link_type}.','
...
sub getAccessibilityData { return $_}
