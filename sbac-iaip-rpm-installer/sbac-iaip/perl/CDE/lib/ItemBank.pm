...
$p{ip_id} ||= $self->{ip_id} || undef;
  ...
  my $sql="SELECT itm.*,u.* FROM item as itm LEFT JOIN user u ON u.u_id = itm.i_author WHERE 1 = 1 ";
...
#get the item from standard value
sub getItemsByStandard{
my ($self, %p) = @_;

    $p{standard_value} ||= $self->{standard_value} || 0;
    
     my @bv    = ( $p{standard_value}, $p{standard_value} );
    $self->{_cfg}->{select_items_by_standard_sql} .= " LIMIT $self->{limit_items}" if $self->{limit_items};
    my $items = $self->{db}->getDataArray(
                        sql => $self->{_cfg}->{select_items_by_standard_sql},
                        values => \@bv,
    );
