...
sub print_welcome {
  ...
  $params->{$_} ||= '' for (qw( doCompare doCompareState itemNotesTag ));
	...
	if ( $userType ne 'psychometrician' || $isAdmin) {
...
END_HERE
	...
	if ( $user->{reviewType} == $UR_DATA_MANAGER && $user->{adminType} == 0) {
		$psgi_out .= <<END_HERE;
       <span><input type="button" class="action_button_long" onClick="window.parent.rightFrame.document.location.href='${javaUrl}Psychometrics.jsf?psychoRenderFlag=render';" value="Psychometrics" /></span>
        <span style="width:${spaceWidth}px">&nbsp;</span>
