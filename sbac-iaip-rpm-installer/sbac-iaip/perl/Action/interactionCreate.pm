...
SQL
  ...
  } elsif ( $in{myAction} eq 'deleteInteraction' ) {

	  $sql = sprintf( "SELECT if_id FROM item_fragment WHERE ii_id=%d", $in{interactionId} );
	  $sth = $dbh->prepare( $sql );  
	  $sth->execute();

	  if ( my $row = $sth->fetchrow_hashref ) {
	    $sql = 'DELETE FROM item_status_fragment WHERE if_id=' . $row->{if_id}; 
	    my $sthStatus = $dbh->prepare($sql);
	    $sthStatus->execute();	    
	  };

    # remove item fragment from the database
    $sql = 'DELETE FROM item_fragment WHERE ii_id=' . $in{interactionId}; 
    $sth = $dbh->prepare($sql);
    $sth->execute();

    # remove item interaction rationale from the database
    $sql = 'DELETE FROM item_interaction WHERE ii_id=' . $in{interactionId}; 
    $sth = $dbh->prepare($sql);
    $sth->execute();
    $in{message} = 'Interaction has been deleted.';
...
sub print_welcome {
  ...
  my $interaction_size = keys %interaction_list;
  my $typeDisplay = &hashToSelect( 'interactionType', \%item_interactions, $ii->{type} );
  my $selectDisplay = &hashToSelect( 'interactionId" id="interactionId', \%interaction_list, $in{interactionId}, , 'null' );
...
<html>
  <head>
	...
	function doDeleteSubmit() {
		if (document.getElementById('interactionId').value != '') {
			if ($interaction_size > 1) {
				if (confirm("Do you want to delete?")) {
		        	document.interactionCreate.myAction.value = 'deleteInteraction';        
					document.interactionCreate.submit();
				}
			} else {
				alert("This is the only interaction of the item, you can not delete.");
			}
		} else {
			alert('Choose interaction first.');
		}
      }
      
      function verifySelection() {
		if (document.getElementById('interactionId').value != '') {
			document.getElementById('deleteInteraction').disabled = false;
		} else {
			document.getElementById('deleteInteraction').disabled = true;
		}
	}
    </script>
  </head>
  <body>
	...
	<td><input type="button" id="deleteInteraction" value="Delete" onClick="doDeleteSubmit();" /></td>
      </tr>
...
END_HERE
        ...
        $psgi_out .= <<END_HERE;
          oEditP.registerEventHandler('ONEDITORLOADED', 'loadEditor0');
END_HERE
    }
...
END_HERE
        ...
        $psgi_out .= <<END_HERE;
          oEditC${j}.registerEventHandler('ONEDITORLOADED', 'loadEditor${jplus}');
END_HERE
    }
...
<!--
  ...
  function loadEditor0() {
   if(typeof(oEditC0) != 'undefined') {
    window.setTimeout(loadTimeOut0, 1);
   }
  }
  function loadTimeOut0() {
    oEditC0.loadEditor();   
  }
  function loadTimeOut1() {
    oEditC1.loadEditor();
  }
  function loadTimeOut2() {
    oEditC2.loadEditor();
  }
  function loadTimeOut3() {
  oEditC3.loadEditor();
  }
  function loadTimeOut4() {
  oEditC4.loadEditor();
  }
  function loadTimeOut5() {
  oEditC5.loadEditor();
  }
  function loadTimeOut6() {
  oEditC6.loadEditor();
  }
  function loadTimeOut7() {
  oEditC7.loadEditor();
  }
  function loadEditor1() {
   if(typeof(oEditC1) != 'undefined') {
    window.setTimeout(loadTimeOut1, 10);
   }
  }
  function loadEditor2() {
   if(typeof(oEditC2) != 'undefined') {
    window.setTimeout(loadTimeOut2, 10);
   }
  }
  function loadEditor3() {
   if(typeof(oEditC3) != 'undefined') {
    window.setTimeout(loadTimeOut3, 10);
   }
  }
  function loadEditor4() {
   if(typeof(oEditC4) != 'undefined') {
    window.setTimeout(loadTimeOut4, 10);
   }
  }
  function loadEditor5() {
   if(typeof(oEditC5) != 'undefined') {
    window.setTimeout(loadTimeOut5, 10);
   }
  }
  function loadEditor6() {
   if(typeof(oEditC6) != 'undefined') {
    window.setTimeout(loadTimeOut6, 10);
   }
  }
  function loadEditor7() {
   if(typeof(oEditC7) != 'undefined') {
    window.setTimeout(loadTimeOut7, 10);
   }
  } 
  function loadEditor8() {
  } 
//-->
</script>
</form>
