...
sub print_item_filter_select {
    ...
    # added by Mithun    
    my $contentAreaDisplay = &hashToSelect( 'hdId1', &getContentArea( $dbh ), $in{'hdId1'},
        'setGLEFilterLevel(\'1\'); return true;', '', '', 'width:150px;' );
    my $itemStandardDisplay;
    if ($in{'hdId1'} ne '') {
      $itemStandardDisplay = 'Standard:&nbsp;&nbsp;';
      $itemStandardDisplay .= &hashToSelect( 'standardId', &selectStandard( $dbh ), $in{'standardId'}, 
	'setGLEFilterLevel(\'1\'); return true;', '', '', 'width:150px;' );
    }
    my $selectHtml = "";
  ...
  <head>
    ...
    <script language="JavaScript">
      ...
      \$(document).ready(function()
			  {
				  \$("#itemTable").tablesorter({headers:{0:{sorter:false},1:{sorter:false}}});
        }
      );
  ...
  <body onLoad="loadCompareStatus();">
    ...
    <table style="border:1px solid black;" cellpadding="3" cellspacing="3">
		  <tr><td>
			...
			<tr><td>Test Subject:&nbsp;&nbsp;
        ${contentAreaDisplay} ${itemStandardDisplay}
			</td>
...
END_HERE
        ...
        my $header = '<th width="1%" border=0" bgcolor="#e6EEEE" align="center">Select</th><th width="2%" bgcolor="#e6EEEE" align="center">Flags</th>';
        $header .= '<th'
          . (
...
sub selectStandard {    
    my %itemStandard;
    $in{'hdId1'} = $in{'hdId1'} eq '' ? 0 : $in{'hdId1'};
    my $sql = "SELECT DISTINCT it.i_primary_standard FROM item it JOIN item_characterization ic"
	." ON it.i_id = ic.i_id AND ic.ic_type = ${OC_CONTENT_AREA} AND ic.ic_value = $in{'hdId1'} AND it.i_primary_standard IS NOT NULL"
	." UNION"
	." SELECT distinct ist.isd_standard FROM item_standard ist"
	." JOIN item it ON ist.i_id = it.i_id"
	." JOIN item_characterization ic  ON it.i_id = ic.i_id AND ic.ic_type = ${OC_CONTENT_AREA} AND ic.ic_value = $in{'hdId1'}";
warn $sql;
    my $sth = $dbh->prepare($sql);
    $sth->execute();

    while ( my $row = $sth->fetchrow_hashref ) {
        $itemStandard{ $row->{i_primary_standard} } = $row->{i_primary_standard};
    }
    $sth->finish;
    return \%itemStandard;
}
...
"SELECT hd_id FROM hierarchy_definition WHERE hd_type=${HD_SS_CONTENT} AND hd_parent_path LIKE '\%,${searchHdId},\%' OR hd_parent_id=${searchHdId}";
    ...
    if($searchHdId) {
      ...
      /*FROM item AS i, item_characterization AS ic, hierarchy_definition AS hd */
      FROM /*cde-filter*/ item AS i join item_characterization AS ic      
      /*WHERE */ ON i.i_id=ic.i_id
        /*AND ic.ic_type=${OC_ITEM_STANDARD}*/
        AND ic.ic_type=${OC_CONTENT_AREA}
	/*AND ic.ic_value=hd.hd_id*/
	/*AND (hd.hd_parent_path LIKE '\%,${searchHdId},\%' OR hd.hd_parent_id=${searchHdId} OR hd.hd_id=${searchHdId})*/
	AND
END_HERE

	$sql .= $in{hdId1} eq '' ? "" : " ic.ic_value = $in{hdId1}"; 
	$sql .= $in{standardId} eq '' ? " AND" : " AND ((i.i_primary_standard = '$in{standardId}')"
		." OR EXISTS (SELECT 1 FROM item_standard isd WHERE isd.i_id = i.i_id"
		." AND isd.isd_standard = '$in{standardId}')) WHERE"; 
    } else {
