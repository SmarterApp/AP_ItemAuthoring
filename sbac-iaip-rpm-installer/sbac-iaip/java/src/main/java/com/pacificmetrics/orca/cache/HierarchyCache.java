...
public class HierarchyCache extends AbstractCache<Integer, String> {
    ...
    private static final Logger LOGGER = Logger.getLogger(HierarchyCache.class
            .getName());

    private static final String HIERARCHY_DEFINITION = "hierarchy_definition";
    ...
    private void load(List<Hierarchy> list) {
        ...
        for (Map.Entry<Integer, Hierarchy> entry : hierarchyMap.entrySet()) {
            map.put(entry.getKey(),
                    getHierarchyAsString(entry.getKey(), hierarchyMap));
        }
    }

    private static String getHierarchyAsString(int id,
            Map<Integer, Hierarchy> hierarchyMap) {
        String result = "";
        ...
        if (hierarchy != null) {
            ...
            result += " /"
                    + StringEscapeUtils.unescapeHtml(hierarchy.getName());
        } else {
