...
sub run {
  ...
  our $itemReviewCall = new Item( $dbh, $in{itemId} );
  if ( $in{saveMetadataAction} eq 'save' ) {
   my $lastDevState = $item->{devState};
    ...
    if ( $in{adminMode} == 2 || $itemReviewCall->{format} == 5) {  
      &setItemReviewState( $dbh, $itemReviewCall->{id}, $lastDevState,
    ...
    <html>
      ...
      <body onLoad="location.href='$thisUrl?myAction=edit&externalId=$in{externalId}&itemBankId=$in{itemBankId}&saved=true';">
      Item Saved!
      </body>
    </html>
END_HERE
    ...
    <html>
      ...
      <body onLoad="location.href='$in{furl}';">
      Item Saved!
      </body>
     </html>
END_HERE
...
sub print_welcome {
  ...
  my $itemFormatHidden = '';
  my $itemFormatDisplay = &hashToSelect( 'itemFormat', \%item_formats, $itemFormat, '', '', '', 'width:165px;' );;
  my $ddlProperty = '';
	if (${itemFormat} == 5) {
		#$ddlProperty = 'pointer-events: none;" id="itemFormat" onkeydown="IgnoreAlpha(event);';
$ddlProperty = '" disabled="disabled';
$itemFormatDisplay =
      &hashToSelect( 'itemFormatTemp', \%item_formats, $itemFormat, '', '', '', 'width:165px;'.$ddlProperty );
	$itemFormatHidden = '<input type="hidden" name= "itemFormat" value="'.${itemFormat}.'" />';
	} else {
		$itemFormatDisplay =
  ...
  my $difficulty_levels = $item->getDifficulty();
  ...
  my $publication_status=$item->getPubStatus();
  ...
  foreach my $type (@ctypes) {
            ...
            and not( $user->{adminType}  ) )
    {
      ...
      if( $type == 5 || $type == 6 ) {
        $charDisplay .= qq|<select name="ct$type" onChange="compareGradeSpan(this.form, this);validateGradeSpan(this.form, this);">|;
      }
      else {
	if ($type == 3) {
        $charDisplay .= qq|<select name="ct$type" onChange="validateGradeSpan(this.form, this);">|;
	}
	else {
...
<html>
  <head>
	...
	// for non Unsupported item format
        if(${itemFormat} != 5){
	  ...
	  if(id == 'tabMain' && i==1 && interactionFrameIsOpen == 0) {
	    ...
	    if(parent.rightFrame == undefined){
	    window.frames['interactionFrame'].location.href='${orcaUrl}cgi-bin/interactionCreate.pl?itemId=${itemId}';
	    ...
	    interactionFrameIsOpen = 1;
          ...
          if(id == 'tabMain' && i==2) {
            // User clicked the Create Graphic tab
            if(parent.rightFrame == undefined){
             window.frames['graphicFrame'].location.href='${assetCreateFramesetUrl}?itemBankId=${ibankId}&itemId=${externalId}&version=${version}';
            }
            else{
          ...
          if(id == 'tabMain' && i==4) {
            // User clicked the Imported Data tab
             if(parent.rightFrame == undefined){
	    window.frames['importedDataFrame'].location.href='${javaUrl}ItemMetadata.jsf?item=${itemId}';	   
	    }
	    else{
       ...
       // User clicked the Notes tab
         if(parent.rightFrame == undefined){
	    window.frames['notesFrame'].location.href='${orcaUrl}cgi-bin/itemNotes.pl?itemId=${itemId}';	   
	    }
	    else{       
       ...
       // User clicked the History tab
        if(parent.rightFrame == undefined){
	    window.frames['historyFrame'].location.href='${orcaUrl}cgi-bin/itemHistory.pl?itemId=${itemId}';	   
	    }
	    else{  
      ...
      if(id == 'tabMain' && i==7) {
          // User clicked the Publication History tab
           if(parent.rightFrame == undefined){
	    window.frames['publicationFrame'].location.href='${javaUrl}ItemPubHistory.jsf?itemId=${itemId}';	   
	    }
	    else{ 
        parent.rightFrame.publicationFrame.location.href='${javaUrl}ItemPubHistory.jsf?itemId=${itemId}'; 
        }
      }
      if(id == 'tabMain' && i==8) {
     // User clicked the Standard tab
      if(parent.rightFrame == undefined){
	    window.frames['standardFrame'].location.href='${javaUrl}ItemStandard.jsf?itemId=${itemId}';	   
	    }
	    else{ 
       parent.rightFrame.standardFrame.location.href='${javaUrl}ItemStandard.jsf?itemId=${itemId}'; 
       }
      }
      ...
      if(id == 'tabMain' && i==9) {
       ...
       if(parent.rightFrame == undefined){
	    window.frames['renderingFrame'].location.href='${javaUrl}ItemAlternate.jsf?itemId=${itemId}';	   
	    }
	    else{ 
...
END_HERE
  ...
  $psgi_out .= <<END_HERE;
    ...
    function validateGradeSpan(f, ct) {	
        if ( f.ct5.selectedIndex > f.ct3.selectedIndex || f.ct6.selectedIndex - 1 < f.ct3.selectedIndex ) {
		alert( 'Grade Level must be within "Grade Span Start" and "Grade Span End"');
		if ( f.ct5.selectedIndex > f.ct3.selectedIndex ) {	
			f.ct5.selectedIndex = 0;
		}
		if ( f.ct6.selectedIndex <= f.ct3.selectedIndex ) {
			f.ct6.selectedIndex = f.ct3.selectedIndex + 1;
		}
		ct.focus();
		}
    }
	...
	return;
	 ...
	 return;
      ...
      if(document.itemCreate.itemDifficulty.value == '0' ||
	   document.itemCreate.itemDifficulty.value == ''
       ) {
	 alert('You must enter a Difficulty.');
	 return;
      }	
      if(document.itemCreate.assignedEditor.value == '0' ||
	   document.itemCreate.assignedEditor.value == ''
       ) {
	 alert('You must enter a Assigned Editor.');
	 return;
      }
      
      document.itemCreate.saveMetadataAction.value = 'save';
      document.itemCreate.submit();
    }
  ...
  <body onLoad="${onLoadJs}">
    <form name="itemCreate" action="${thisUrl}" method="POST">
      <input type="hidden" name="itemNote" value="" />
      <input type="hidden" name="myAction" value="" />
      <input type="hidden" name="saveMetadataAction" value="" />
...
if(${itemFormat} == 5){
$psgi_out .= <<END_HERE;
&#160;&#160;<input type="button" value="Save Item" onClick="return doSaveSubmit();"/>
END_HERE
}
...
# It will not display for Unsupported Item Format
	 ...
	 <table class="no-style" border="0" cellpadding="2" cellspacing="2">
	   ...
	   <br />
           <table class="no-style" border="0" cellpadding="2" cellspacing="2">	    
             <tr>
               <td>Notes:</td>
               <td><textarea id="item_notes" name="itemNotes" rows="6" cols="53">${notes}</textarea></td>
             </tr>
           </table>
         </td> 
...
END_HERE
  ...
  $psgi_out .= <<END_HERE;
         ...
         <tr><td colspan="2">&#160;</td></tr>
           ...
           <td>Assign:</td>
           <td>
    <input type="button" onClick="myOpen('assignStandardWindow','${assignStandardUrl}?itemId=${itemId}',594,480);" value="Standard" style="display:none;"/>
          &#160;&#160; 
...
#It will display for Unsupported Item Format
  $psgi_out .= <<END_HERE;
    ...
    <td><input type="text" size="42" name="itemDescription" value="${descrip}" /></td>
      ...
      <tr>
        <td>Item Format:</td>
        <td>${itemFormatDisplay}${itemFormatHidden}</td>
      </tr>   
      <tr>
   ...
   <td style="align:right; vertical-align:top;">
    ...
    <td>${languageDisplay}</td>
      ...
      <tr>
        <td>Due Date:</td>
    <td>
       <input type="text" id="dueDate" name="dueDate" size="11" value="$due_date" readonly="readonly" onclick="javascript:showCal('calendar1')" />
...
END_HERE
#Standadrd Tab start here

$psgi_out .= <<END_HERE;
<div class="tabbertab" title="Standard">
       <iframe name="standardFrame" id="standardFrame" width="97%" height="600" frameborder="0" scrolling="auto" src="${orcaUrl}blankPage.html">Standard</iframe>
      </div>

END_HERE

#Standard tab end here
...
$psgi_out .= <<END_HERE;
    ...
    var savedFlag = document.itemCreate.savedFlag.value;
	if('true' == savedFlag) {
		alert('Item Saved!');
	}
    <!--
