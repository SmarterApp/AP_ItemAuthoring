...
sub run {
  ...
  # version of itself
      ...
      if ( $in{itemBankId} eq '' or $in{targetBankId} eq '' ) {
          ...
          $sql =
            "SELECT p_id, p_name FROM passage WHERE ib_id=$in{targetBankId} AND p_name IN"
            . " (SELECT p_name FROM passage WHERE p_id IN ("
            . join( ',', @passageIdArray ) . "))";
          $sth = $dbh->prepare($sql);
          $sth->execute();
          if ( my $row = $sth->fetchrow_hashref ) {
          		$passage_name = $row->{p_name}.", ";
              
				while ($row = $sth->fetchrow_hashref ) {
					$passage_name .= $row->{p_name}.", ";
				}
              $in{message} =
                "Cannot Move Passages, because these names ( ".$passage_name.") already exist in target bank.";
              #return [ $q->psgi_header('text/html'), [ &print_welcome(\%in) ] ];
          }
          ...
          foreach my $passageId (@passageIdArray) {
          		...
          		$sql = "SELECT p_url, ib_id, p_name FROM passage WHERE ib_id = $in{itemBankId} AND p_name NOT IN " 
		. "(SELECT p_name FROM passage WHERE ib_id = $in{targetBankId} AND p_name IN "
		. "(SELECT p_name FROM passage WHERE p_id IN (${passageId}))) AND p_id IN (${passageId})";
  			...
  			if($flag eq 1) {
		          $in{message} .=
		            "</br>Moved Passages ( " . $passage_name . ") successfully.";
			  }
          #$in{message} = "Moved " . scalar(@passageIdArray) . " Passages successfully.";
      }
  }
...
#added for edit(Select menu)
    $sqlTemp = "SELECT DISTINCT i.i_format FROM item as i, item_characterization as ic WHERE ic.ic_type = ${OT_ITEM} AND ic.ic_value = ? AND i.i_id = ic.i_id"; 
