...
sub run {
  ...
  # version of itself
      ...
      if ( $in{itemBankId} eq '' or $in{targetBankId} eq '' ) {
          ...
          $sql =
  			"SELECT sr_id, sr_name FROM scoring_rubric WHERE ib_id=$in{targetBankId} AND sr_name IN"
            . " (SELECT sr_name FROM scoring_rubric WHERE sr_id IN ("
            . join( ',', @rubricIdArray ) . "))";
          $sth = $dbh->prepare($sql);
          $sth->execute();
          if ( my $row = $sth->fetchrow_hashref ) {
          	$rubric_name = $row->{sr_name}.", ";
              
			while ($row = $sth->fetchrow_hashref ) {
				$rubric_name .= $row->{sr_name}.", ";
			}
              $in{message} =
                "Cannot Move Rubrics, because these names ( ".$rubric_name.") already exist in target bank.";
              #return [ $q->psgi_header('text/html'), [ &print_welcome(\%in) ]];
          }
          ...
          foreach my $rubricId (@rubricIdArray) {
          	  $sql = "SELECT sr_url, ib_id, sr_name FROM scoring_rubric WHERE ib_id = $in{itemBankId} AND sr_name NOT IN
			(SELECT sr_name FROM scoring_rubric WHERE ib_id = $in{targetBankId} AND sr_name IN
			(SELECT sr_name FROM scoring_rubric WHERE sr_id IN (${rubricId}))) AND sr_id IN (${rubricId})";
              #$sql = "SELECT sr_url, ib_id FROM scoring_rubric WHERE sr_id=${rubricId}";
              $sth = $dbh->prepare($sql);
              $sth->execute();
          ...
          #$in{message} = "Moved " . scalar(@rubricIdArray) . " Rubrics successfully.";
          if($flag eq 1) {
	          $in{message} .=
	            "</br>Moved Rubrics ( " . $rubric_name . ") successfully.";
		  }
      }
  }
