...
sub displaySearchMenu {
	...
	$p{item_bank} = {};
      ...
      #my $standard_list = $item->getStandardList(); 
	  #$p{i_standard_list} = $self->{_CDE_}->_hashToSelectList(hash =>  $standard_list, 
    	#);
      $p{i_standard_list} = $item->getStandardList();
    ...
    $template->param( metadata_headers => $self->{_cfg}->{metadata_headers},
		      ...
		      standard_keys => $self->{_cfg}->{standard_keys},
		      statistics_keys => $self->{_cfg}->{statistics_keys},
    );
...
sub getItems {
	...
	if( $self->{cgi_params}->{subaction} eq 'search_standards' ) {
         ...
         $items = $self->{_IB_}->getItemsByStandard( standard_value => $self->{cgi_params}->{standard_value} );
...
sub moveItems {
    ...
    for ( @item_list ) {
        ...
        if( $mutable->{cnt} == 0 ) {
            my $item_exist = $self->{_CDE_}->{db}->getDataHash(
                                sql => $self->{_cfg}->{select_item_exist_sql},
	    ...
	    if( $item_exist->{cnt} == 0 ) {
            	...
            	$moved_item_name .= $item->{name}.", ";
            	$move_cnt++;
	    } else {
			my $item = new Item( $self->{_CDE_}->{db}->{dbh}, $_->{i_id} );
			$rest_item_name .= $item->{name}.", ";
			$flag = 1;
	    }
        }
    }
	...
	$p{msg}       = sprintf "%d Item(s) ( ".$moved_item_name." ) Successfully Moved!", $move_cnt;
    if($flag eq 1) {
    	$p{msg}	.= "</br> Some items ( ".$rest_item_name." ) already exist in target bank.";
    }
...
sub getPassageByFormatLists {
    ...
    my %tbl;
    my $passages = $self->{_CDE_}->{db}->getDataArray(
                                sql => $self->{_cfg}->{select_passage_by_program_and_format_sql},
                                values => [$self->{cgi_params}->{ib_id}],
    );
    $passages = [ { p_name => '---', p_id => 0 } ] if( ref $passages eq 'HASH' );
    $tbl{passages} = $passages;
