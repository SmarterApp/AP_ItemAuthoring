...
sub new {
    ...
    if ( $self->{id} > 0 ) {
        ...
        if ( my $row = $sth->fetchrow_hashref ) {
            ...
            $self->{primarystandard}   = $row->{i_primary_standard};
...
my ($self) = shift;
my %standardList = ();
$sql="SELECT DISTINCT itm.i_primary_standard standard FROM item itm WHERE itm.i_primary_standard IS NOT NULL UNION SELECT DISTINCT its.isd_standard standard FROM item_standard its ORDER BY 1";
    my $sth = $self->{dbh}->prepare($sql) || return $self->error( "Failed Query:" . $self->{dbh}->err );
        $sth->execute();
        $p{s} ||= '';
        while ( my $row = $sth->fetchrow_hashref ) {
           #$standardList{ $row->{standard} } = $row->{standard};
           push @standardList, { _value => $row->{standard}, _label => $row->{standard}, _selected => $p{s} eq $row->{standard} ? 'selected' : '' }; 
           }
     $sth->finish;
   return \@standardList;
}
...
my ($self) = shift;
my %adminList = ();
$sql="Select * from stat_administration ";
    my $sth = $self->{dbh}->prepare($sql) || return $self->error( "Failed Query:" . $self->{dbh}->err );
        $sth->execute();
        while ( my $row = $sth->fetchrow_hashref ) {
           $adminList{ $row->{sa_id} } = $row->{sa_administration};
...
$sql="Select * from difficulty ";
    my $sth = $self->{dbh}->prepare($sql) || return $self->error( "Failed Query:" . $self->{dbh}->err );
        $sth->execute();
        while ( my $row = $sth->fetchrow_hashref ) {
           $difficulty_level{ $row->{d_id} } = $row->{d_name};
...
#added for pub status
 sub getPubStatus{
  my ($self) = shift;
  my %pub_status = ();
  $i=1;
  $sql="Select * from publication_status";
    my $sth = $self->{dbh}->prepare($sql) || return $self->error( "Failed Query:" . $self->{dbh}->err );
        $sth->execute();
        while ( my $row = $sth->fetchrow_hashref ) {
           $pub_status{ $row->{ps_id} } = $row->{ps_name};
...
sub setPrimaryStandard         { $_[0]->{primarystandard}       = $_[1]; }
...
sub getDisplayContent {
    ...
    $sql = 'SELECT ae.i_id, ae.ae_content_name, ae.ae_content_link_type, 
            ...
            where ae.i_id = '. $self->{id} . ' and af.af_type = 1 and af.af_feature = 2';
    ...
    while(my $row = $sth->fetchrow_hashref){
        $c->{accessibilityData} .=  '{"name":"'.$row->{ae_content_name}.'","linkType":'.$row->{ae_content_link_type}.','
    ...
    $c->{metadata} = sprintf
                        qq| <br/>
                         <table border="1" cellspacing="2" cellpadding="2">
                            <tr><td width="150px">Primary Standard</td><td>%s</td></tr>
...
sub create {
    ...
    $sql =
"INSERT INTO item (ib_id, i_external_id, i_dev_state, i_xml_data, i_created,i_primary_standard) "
      . "VALUES (${bank},"
      . $self->{dbh}->quote($name) . ",1,"
      . $self->{dbh}->quote($xml)
      . ",NOW(),".$self->{dbh}->quote($pstandard).")";
    $sth = $self->{dbh}->prepare($sql);
    $sth->execute() || return 0;
