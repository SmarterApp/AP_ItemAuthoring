...
@Startup
@Singleton
public class SAAIFFTPScheduler {

	private static final Log LOGGER = LogFactory
			.getLog(SAAIFFTPScheduler.class);

	private static final String INPROGRESS_FILE = "content_import_monitor.inprogress";
	
	@Resource
	private TimerService timerService;

	@EJB
	private SAAIFFTPImporter saaifFtpImporter;

	@PostConstruct
	public void initTimer() {
		LOGGER.info("SAAIFFTPScheduler bean started ...");
		...
		LOGGER.info("SAAIFFTPScheduler completed creating scheduler with expression "
	...
	public void timeout(Timer timer) {
		...
		if (createInProgressFile(tempPath)) {
			LOGGER.info("Invoking SAAIFFTPImporter ...");
			saaifFtpImporter.importFromFTP(sourcePath);
			LOGGER.info("Completed SAAIFFTPImporter task ...");
			removeInProgressFile(tempPath);
		} else {
	...
	public SAAIFFTPImporter getSaaifFtpImporter() {
		return saaifFtpImporter;
	}

	public void setSaaifFtpImporter(SAAIFFTPImporter saaifFtpImporter) {
		this.saaifFtpImporter = saaifFtpImporter;
	}
