...
var Calendar = new Class({
	...
	initialize: function(el, toggler, options){
		...
		this.preTD.addEvent("click", function(){
			...
			var new_dt;
			while(i > 0){
				new_dt = new Date(this.date.getFullYear(), this.date.getMonth()-i, this.options.startDate.getDate());
	...
	*/
	processDates: function(dt, use_date){
		...
		if ((($type(dt) == "object") && ($defined(dt.getFullYear))) || ($type(dt) == "date")) return dt;
		else if ($type(dt) == "object") return use_date.fromObject(dt);
		else if ($type(dt) == "string") return use_date.fromString(dt);
		else if ($type(dt) == "number") return new Date(dt);
		else return null;
	},
