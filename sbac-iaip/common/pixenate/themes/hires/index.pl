...
[%
 ...
 my $image = "/pixenate/images/samples/hongkong.jpg";
 if ($request->param("image") ne ""){
	$image = $request->param("image");
 }

 my $originalFilename = "";

 if ($request.param("originalFilename") ne ""){
	$originalFilename = $request->param("originalFilename");
 }
%]
<body bgcolor="white">
<script type="text/javascript">
pxn8_original_filename = "[% print $originalFilename; %]";
PXN8.dom.addLoadEvent(function(){
	PXN8.initialize('[% print $image; %]');
	/**
...
<script type="text/javascript">
 ...
 my $hires_image = "/pixenate/images/samples/hires/hongkong.jpg";
 if ($request->param("hires_image") ne ""){
	$hires_image = $request->param("hires_image");
 }
PXN8.hires.init("[% print $hires_image %]");
...
<input type="hidden" name="next_page" value="/pixenate/themes/hires/index.pl"/>
