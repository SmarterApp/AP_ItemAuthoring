...
function showCal(name) {
  ...
  if (currentCal != null && currentCal.form != null && currentCal.form[currentCal.field]) {
    ...
    if (!hasCal) {
      if (isIE || isOpera6) {
        calWin=window.open("blank.htm","cal","toolbar=0,width="+calWidth+",height="+calHeight+",left="+(winX+calOffsetX)+",top="+(winY+calOffsetY));
      }
      else {
        calWin=window.open("blank.htm","cal","toolbar=0,width="+calWidth+",height="+calHeight+",screenx="+(winX+calOffsetX)+",screeny="+(winY+calOffsetY));
      }
    }
