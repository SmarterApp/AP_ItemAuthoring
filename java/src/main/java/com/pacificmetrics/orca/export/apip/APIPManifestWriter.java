...
import com.pacificmetrics.apip.cp.manifest.ResourceMetaType;
...
public class APIPManifestWriter {
    private static final Logger log = Logger.getLogger(APIPManifestWriter.class.getName());
    ...
    public void addItem(APIPItem item) throws XPathExpressionException {
        ...
        // was building metadata for item inside the item resources definition in the manifest
        /*
        // create the RMT (<metadata> tag) and add it to item resource
        ResourceMetaType rmt = new ResourceMetaType();
        resource.setResourceMeta(rmt);

        // create the LOM (<lom>...</lom> metadata structure) and add it to the RMT (<metadata> tag)
        LomResourceType lomr = mkResourceLom(item, "nothing special");
        rmt.setLom(lomr);
        */
        
        item.setMetadataHref(getItemMetadataHref(item.getId(), item.getId() + "_metadata.xml"));
        item.setMetadataHrefBase(getItemMetadataHrefBase(item.getId()));
        
        // add item metadata to manifest resources
        /*
        <resource identifier="metadata_SQA094" type="controlfile/apip_xmlv1p0" href="metadata/item_SQA094/item_SQA094_metadata.xml">
            <file href="metadata/item_SQA094/item_SQA094_metadata.xml" />
        </resource>
         */
        ...
        // AIR TIB DEPENDENCY Must start with "metadata" prefix (ApipZipInputFileExtractorService.META_DATA_FILE_PREFIX)
        metadataResource.setId("metadata_" + item.getId());
        this.m.getResources().add(metadataResource);
        
        // add metadata file to metadata resource
        ...
        // 2. add metadata to item dependency
        /*
         <dependency identifierref="metadata_SQA094" />
         */
        dependencies.add("metadata_" + item.getId());
        
        for (String r : item.getResources()) {
log.info(item.getHrefBase()+r);
    ...
    public void addDefaultManifestMetadata(String version, String description, String packageName) {
        ...
        CopyrightAndOtherRestrictionsType copy = this.of.createCopyrightAndOtherRestrictionsType();
    ...
    public final void write(OutputStream outputStream) throws JAXBException, FileNotFoundException, TransformerException {
        ByteArrayOutputStream os = new ByteArrayOutputStream();
        JAXBContext jc = JAXBContext.newInstance(this.m.getClass().getPackage().getName());
        ...
        Transformer transformer = tFactory.newTransformer(new StreamSource(APIPManifestWriter.class.getResourceAsStream("/xslt/apip/manifest.xsl")));
        transformer.setOutputProperty(OutputKeys.INDENT, "yes");
        transformer.transform(new StreamSource(new ByteArrayInputStream(xml.getBytes())), new StreamResult(outputStream));
    }
    ...
    private LomResourceType mkResourceLom(APIPItem item, String description) throws XPathExpressionException {
    ...
    private final String getUniqueId() {
        String key = 'A'+UUID.randomUUID().toString().substring(1).toUpperCase();
