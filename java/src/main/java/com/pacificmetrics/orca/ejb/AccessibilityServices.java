...
public class AccessibilityServices implements Serializable {
    ...
    public List<AccessibilityElement> findAccessibilityElements(int itemId) {
    ...
    public void deleteAccessibilityElements(int itemId, List<String> retainElementNames) {
    ...
    public void replaceAccessibilityElements(int itemId, List<AccessibilityElement> elements) {
        replaceAccessibilityElements(itemId, elements, Collections.<String>emptyList());
    }
	
	public void replaceAccessibilityElements(int itemId, List<AccessibilityElement> elements, Collection<String> modifiedElementNames) {
		//delete those elements that have been removed by user; populating array of element names to retain
		...
		for (AccessibilityElement ae: elements) {
			if (!existingElementNames.contains(ae.getName())) {
			    ...
			    AccessibilityElement elementToModify = elementsToModify.get(ae.getName());
	...
	public void updateElement(AccessibilityElement elementToModify, AccessibilityElement newElement) {
	 ...
	 * 
	 * @param featuresMap Map elementId to List of features
	 * 
	 */
	public void replaceAccessibilityFeatures(Map<Integer, List<AccessibilityFeature>> featuresMap) {
		for (Map.Entry<Integer, List<AccessibilityFeature>> entry: featuresMap.entrySet()) {
			replaceAccessibilityFeatures(entry.getKey(), entry.getValue());
		}
	}
	
	public void replaceAccessibilityFeatures(int elementId, List<AccessibilityFeature> features) {
	...
	public List<Integer> getFeaturesForType(int type) {
		...
		switch (type) {
		case AccessibilityFeature.T_BRAILLE: result.add(AccessibilityFeature.F_BRAILLE_TEXT); break;
		case AccessibilityFeature.T_HIGHLIGHTING: result.add(AccessibilityFeature.F_HIGHLIGHTED_TEXT); break;
		case AccessibilityFeature.T_SPOKEN: /*result.add(AccessibilityFeature.F_AUDIO_FILE);*/ //Commented out for DE1054, as it is not needed just yet
											result.add(AccessibilityFeature.F_AUDIO_TEXT);
											result.add(AccessibilityFeature.F_TEXT_TO_SPEECH); break; 
		case AccessibilityFeature.T_TACTILE: result.add(AccessibilityFeature.F_AUDIO_FILE);
											 result.add(AccessibilityFeature.F_AUDIO_TEXT);
											 result.add(AccessibilityFeature.F_BRAILLE_TEXT); break; 
		}
		return result;
	}
	
	public AccessibilityFeature insertFeature(int featureId, int elementId, int featureType, int featureCode, String langCode, String info) throws ServiceException {
		AccessibilityFeature feature = featureId > 0 ? findAccessibilityFeatureById(featureId) : new AccessibilityFeature();
		if (feature == null) {
			throw new ServiceException(AccessibilityServicesStatus.FEATURE_NOT_FOUND, featureId);
		}
		feature.setElementId(elementId);
	...
	public void deleteAccessibilityFeature(int id) throws ServiceException {
		...
		if (feature == null) {
			throw new ServiceException(AccessibilityServicesStatus.FEATURE_NOT_FOUND, id);
		}
		entityManager.remove(feature);
	}
	
	public void persistInclusionOrders(Collection<InclusionOrder> inclusionOrders) {
	...
	public void replaceInclusionOrders(int id, Collection<InclusionOrder> inclusionOrders) {
    ...
    private Query createNamedQuery(String queryName) {
        return entityManager.createNamedQuery(queryName.replace("?", getEntityName()));
    }
