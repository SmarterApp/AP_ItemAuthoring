...
//	public void updateMetafile(ItemBankMetafile ibm) {
//		ItemBankMetafile existingIBM = findMetafileById(ibm.getId());
//		// newIBM - new version of the metafile. version incremented by one;
//		// original id from previous version
//		ItemBankMetafile newIBM = addMetafile(ibm.getItemBankId(), ibm.getOriginalFileName(), existingIBM.getVersion() + 1,
//				                              existingIBM.getFileType(), existingIBM.getComment(), existingIBM.getId());
//		existingIBM.setVersion(existingIBM.getVersion() + 1);
//		existingIBM.setOriginalFileName(ibm.getOriginalFileName());
//		existingIBM.setFileType(ibm.getFileType());
//		existingIBM.setComment(ibm.getComment());
//		if (entityManager != null) {
//			entityManager.merge(existingIBM);
//		}
...
//	}
	...
	public ItemBankMetafile addMetafile(int ibId, String fileName, int version, String fileType, String comment, Integer metafileId, int typeCode) throws ServiceException {
		...
		if (metafileId == null) {
			metafileId = getMaxMetafileId() + 1;
		}
		ibm.setId(metafileId);
		...
		ibm.setSystemName(metafileId + "." + version + "-" + fileName);
