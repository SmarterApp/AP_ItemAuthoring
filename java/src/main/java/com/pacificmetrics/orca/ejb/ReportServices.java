...
public class ReportServices {
    ...
    static private Logger logger = Logger.getLogger(ReportServices.class.getName());
    ...
    public void init() {
        ...
        reportsDirectory = ServerConfiguration.getProperty("jasper.reports.directory");
    ...
    public byte[] getReportAsByteArray(Report report) throws IOException, DocumentException {
        ...
        String reportUrl = report.getUrl().replaceFirst("(?i)/reports", "/reports" + reportsDirectory);
        
        String describeResourcePath = "/resource" + reportUrl;
        String generateReportPath = "/report" + reportUrl + "?RUN_OUTPUT_FORMAT=" + report.getFormat();
        ...
        try {
            ...
            logger.warning("Report Exception: " + e.getMessage());
            return new byte[0];
        }

        byte[] bytes = resource.get(byte[].class);
        logger.info("Report received. Size=" + (bytes != null ? bytes.length : 0));
        
        return bytes;
    }

    private Document parseResource(String resourceAsText) throws DocumentException {
        Document document;
        document = DocumentHelper.parseText(resourceAsText);
        return document;
    }

    private String parseReport(String reportResponse) throws DocumentException, ReportException {
