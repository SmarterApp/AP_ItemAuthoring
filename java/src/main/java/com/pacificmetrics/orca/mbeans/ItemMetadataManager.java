...
@RequestScoped
public class ItemMetadataManager extends AbstractManager implements Serializable
{
     ...
     * Redirects to error page if item not specified or not found, if there's no metadata 
     * associated with the item or metadata cannot be parsed
     * XMLTreeView is constructed from the parsed XML document
     * populateTreeData() method is called to populate tree data from XMLTreeView 
     *   
     */
    @PostConstruct
    public void load() {
    ...
    private void populateTreeData(XMLTreeNode xmlTreeNode, TreeNode treeNode) {
        ...
        for (XMLTreeNode xmlChildNode: xmlTreeNode.getChildren()) {
            TreeNodeBase childNode = new TreeNodeBase(xmlChildNode.hasChildren() ? "folder" : "document", 
