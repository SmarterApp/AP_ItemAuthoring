...
@ViewScoped
public class IBMetafileAssociationManager extends AbstractManager implements Serializable {
	
    static private Logger logger = Logger.getLogger(IBMetafileAssociationManager.class.getName()); 
	...
	transient private HierarchyCache hierarchyCache;
...
//	private List<ItemMetafileAssociation> existingItemAssociations;
//	private List<ItemMetafileAssociation> outdatedItemAssociations;
	...
	public void load() {
        ...
        CacheManager cache = findBean("cache"); //TODO find out why ManagerProperty don't work
        if (cache == null) {
	...
	public void confirmNewIDs() {
		...
		if (splitNewIDs.length == 0 || (splitNewIDs.length == 1 && splitNewIDs[0].trim().isEmpty())) {
			error(IBMetafileServicesStatus.NOTHING_TO_PROCESS);
			return;
		}
		OperationResult res = metafileServices.associateItemsWithMetafile(metafileId, version, splitNewIDs);
		if (res.isSuccess()) {
			int count = 0;
			for (Map.Entry<String, Status> entry: ((MultipleResults<String>)res).getStatusMap().entrySet()) {
	...
	public String getPrimaryHierarchy(Item item) {
		...
		ItemCharacterization ic = item.getCharacterization(1); //TODO make constant
		return ic != null && hierarchyCache != null ? hierarchyCache.get(ic.getIntValue()) : "";
	}
	...
	public void updateSelectedAssociations() {
		...
		for (Map.Entry<Integer, Boolean> entry: outdated.associationsChecked.entrySet()) {
			if (entry.getValue()) {
	...
	private void updateAssociations(List<Integer> associationsToUpdate) {
		...
		OperationResult res = metafileServices.updateItemAssociations(metafileId, version, associationsToUpdate);
		if (res.isSuccess()) {
			int count = 0;
			for (Map.Entry<Integer, Status> entry: ((MultipleResults<Integer>)res).getStatusMap().entrySet()) {
				if (entry.getValue() == Status.OK) {
					count++;
				} else {
					error(entry.getValue(), entry.getKey());
	...
	public void removeSelectedAssociations() {
		...
		for (Map.Entry<Integer, Boolean> entry: existing.associationsChecked.entrySet()) {
			if (entry.getValue()) {
	...
	private void removeAssociations(List<Integer> associationsToRemove) {
		OperationResult res = metafileServices.unassociateItems(metafileId, version, associationsToRemove);
		if (res.isSuccess()) {
			int count = 0;
			for (Map.Entry<Integer, Status> entry: ((MultipleResults<Integer>)res).getStatusMap().entrySet()) {
				if (entry.getValue() == Status.OK) {
	...
	public abstract class Records implements Serializable {
		...
		public int getDisplayedRecordCount() {
			return maxDisplayRecordCount < getTotalRecordCount() ? maxDisplayRecordCount : getTotalRecordCount();
		}
		...
		public int getTotalRecordCount() {
			List<ItemMetafileAssociation> itemAssociations = getItemAssociations();
			return itemAssociations != null ? itemAssociations.size() : 0;
		}
	...
	public String getItemViewURL() {
		return ServerConfiguration.getProperty(ServerConfiguration.HTTP_SERVER_CGI_BIN_URL)+ "/itemView.pl";
	}
