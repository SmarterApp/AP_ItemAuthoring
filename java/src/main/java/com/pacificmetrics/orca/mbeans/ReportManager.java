...
public class ReportManager extends AbstractManager implements Serializable {
    ...
    static private Logger logger = Logger.getLogger(ReportManager.class.getName()); 
...
//    private Map<String, String> reportParameters = new HashMap<String, String>();
    ...
    @EJB
    transient private ReportServices reportServices;
    
    @PostConstruct
    public void load() throws ManagerException {
        reportName = getParameter("name", "");
        saveAsFormat = getParameter("output", ServerConfiguration.getProperty("jasper.default.output"));
    ...
    private String getReportParametersAsString() {
        ...
        for (Map.Entry<String, String> entry: parametersMap.entrySet()) {
            if (entry.getKey().startsWith("p_")) {
                result += "&" + entry.getKey().substring(2) + "=" + entry.getValue();
            }
        }
        return result;
    }
    ...
    /**
     * parameters that have to be passed to the report are derived from page parameters that have prefix p_
     * @param report
     */
    private void addParameters(Report report) {
        for (Map.Entry<String, String> entry: parametersMap.entrySet()) {
            if (entry.getKey().startsWith("p_")) {
                report.addParameter(entry.getKey().substring(2), entry.getValue());
            }
        }
    }
    ...
    public void generate2(PhaseEvent event) throws ManagerException {
        ...
        String requestURL = url + reportName + //"&j_username=" + userName + "&j_password=" + password + 
        		"&output=" + saveAsFormat + getReportParametersAsString();
        
        String fileName = reportName.substring(reportName.lastIndexOf('/') + 1) + "." + saveAsFormat;
        
        try {
            URLConnection con = new URL(requestURL).openConnection();
            //String str = IOUtils.toString(con.getInputStream());
            byte[] bytes = IOUtils.toByteArray(con.getInputStream());
            
            FileUtils.writeByteArrayToFile(new File("/temp/" + fileName), bytes);
            ...
            ec.setResponseContentType(ec.getMimeType(fileName)); // Check http://www.w3schools.com/media/media_mimeref.asp for all types. Use if necessary ExternalContext#getMimeType() for auto-detection based on filename.
...
//        jasper.user.name=sbac1
//                jasper.password=sbac11
//                jasper.default.output=csv

    }
    
    private byte[] generateReport(String format) throws ManagerException {
        System.out.println("About to generate report " + reportName);
    ...
    public void doSave() throws ManagerException {
        System.out.println("Saving as " + saveAsFormat + "...");
        try {
