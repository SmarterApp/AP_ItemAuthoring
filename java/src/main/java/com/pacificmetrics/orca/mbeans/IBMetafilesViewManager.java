...
public class IBMetafilesViewManager {
	...
	public void load() {
        ...
        if (itemParamValue != null) {
        	...
        	associations = metafileServices.getMetafileAssociationsForItem(itemId);
        	latestVersionsMap = metafileServices.getLatestVersionsMapForAssocations(associations);
        	...
        	associations = metafileServices.getMetafileAssociationsForPassage(passageId);
        	latestVersionsMap = metafileServices.getLatestVersionsMapForAssocations(associations);
        }
        if (associations != null) {
        	System.out.println("Associations loaded: " + associations.size());
        }
	}
	...
	public boolean displayWarning(MetafileAssociation association) {
		Integer latestVersion = latestVersionsMap.get(association.getMetafileId());
		return latestVersion != null && latestVersion > association.getVersion();
	}
