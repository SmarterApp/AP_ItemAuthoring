...
public class AbstractManager implements Serializable {
	...
	static private Logger logger = Logger.getLogger(AbstractManager.class.getName()); 
	...
	protected void initializeParameters() {
		...
		if (context != null) {
    		initializeParameters(new HashMap<String, String>(context.getExternalContext().getRequestParameterMap()));
		} else {
    ...
    protected void redirectWithErrorMessage(String message) {
        redirectWithError(this.messageContext.message().text("{" + message + "}").toText());
    }
	...
	protected void error(String msgKey) {
		Message message = this.messageContext.message().text("{" + msgKey + "}").create();
		this.messageContext.addMessage(message);
	}
	...
	protected void error(String msgKey, String text) {
		String msgText = this.messageContext.message().text("{" + msgKey + "}").toText();
		Message message = this.messageContext.message().text(msgText + " : " + text).create();
		this.messageContext.addMessage(message);
	}
	
	protected void error(Status status1, Status status2) {
		String text1 = this.messageContext.message().text("{" + status1.toString() + "}").toText();
		String text2 = this.messageContext.message().text("{" + status2.toString() + "}").toText();
		Message message = this.messageContext.message().text(text1 + " : " + text2).create();
		this.messageContext.addMessage(message);
	}
	...
	protected <T> void handleOperationResult(OperationResult res) {
		if (res.isSuccess()) {
			...
			if (res instanceof MultipleResults) {
				for (Map.Entry<T, Status> entry: ((MultipleResults<T>)res).getStatusMap().entrySet()) {
	...
	public static <T> T findBean(String beanName) {
	    return (T)FacesContext.getCurrentInstance().getELContext().getELResolver().getValue(FacesContext.getCurrentInstance().getELContext(), null, beanName);
	}
