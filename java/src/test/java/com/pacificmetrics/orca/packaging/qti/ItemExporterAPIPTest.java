...
public class ItemExporterAPIPTest {
    ...
    private static final String Item_VE_IP_01 = IMS_DIR + "Item_VE_IP_01/apipv1p0_EntryTest_VE_IP_01.xml";
    private static final String Item_VE_IP_02 = IMS_DIR + "Item_VE_IP_02/apipv1p0_EntryTest_VE_IP_02.xml";
    private static final String Item_VE_IP_03 = IMS_DIR + "Item_VE_IP_03/apipv1p0_EntryTest_VE_IP_03.xml";
    private static final String Item_VE_IP_04 = IMS_DIR + "Item_VE_IP_04/apipv1p0_EntryTest_VE_IP_04.xml";
    private static final String Item_VE_IP_05 = IMS_DIR + "Item_VE_IP_05/apipv1p0_EntryTest_VE_IP_05.xml";
    ...
    public final void manifestTest(){
        assertEquals(ItemExporterAPIPTest.manifestWriterAPIP, ex.getManifestWriter());
    }
    ...
    /**
     * Test method for {@link com.pacificmetrics.orca.export.apip.APIPItemExporter#export(java.util.List)}.
     * 
     ...
     */
    @Test
    public final void testIMSExport() throws ItemExportException, IOException, URISyntaxException {
        ex.setRemoteContentBase(new File(getClass().getResource(IMS_DIR).toURI()));
        ex.initialize();
    ...
    @Test
    public final void testBadXMLExport() throws ItemExportException, IOException, URISyntaxException {
        // Update remote content base for resolution of CDE resources. Only media files are currently pulled
        ex.setRemoteContentBase(new File(getClass().getResource(CDE_DIR).toURI()));
        ex.initialize();
        try {
            ...
            assertTrue(e.getMessage().contains("There are multiple occurrences of ID value 'content2'"));
        }
        finally {
            ex.destroy();
        }
    ...
    @Test
    public final void testBadSchematronExport() throws ItemExportException, IOException, URISyntaxException {
        ex.setRemoteContentBase(new File(getClass().getResource(CDE_DIR).toURI()));
        ex.initialize();
        try {
            ...
            assertTrue(e.getMessage().contains("The StringIdentifier and ResponseIdentifier attribute values must not be the same."));
            assertTrue(e.getMessage().contains("Invalid number of \"calculator\" elements"));
        }
        finally {
            ex.destroy();
        }
    ...
    @Test
    public final void testGoodExport() throws ItemExportException, IOException, URISyntaxException {
        ex.setRemoteContentBase(new File(getClass().getResource(CDE_DIR).toURI()));
        ex.initialize();
    ...
    private List<Item> getCDEItems(String id, String xml) throws IOException, URISyntaxException {
        final List<Item> items = new ArrayList<Item>(1);
        items.add(getItem(id, xml, 15));
        return items;
    }

    private Item getItem(String id, String xml, int bankId) throws IOException, URISyntaxException {
        Item item = new Item();
        item.setQtiData(FileUtils.readFileToString(new File(getClass().getResource(xml).toURI()), "UTF-8"));
